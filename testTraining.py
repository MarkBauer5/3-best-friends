...
def main():
    ...
    def trainEpoch(model:nn.Module, dataloader:DataLoader, scheduler:WarmupPlataeuScheduler, freezeModel):
        ...
        for batchNum, (images, labels) in zip(pbar, dataloader):
            ...
            pbar.set_description("loss: {:.6f}, lr: {:.6f}, accuracy: {:.5f}".format(loss, currentLr, correct/total), refresh=True)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        trainLoss, trainAccuracy = trainEpoch(model, trainLoader, scheduler, freezeModel=False)
        with torch.no_grad():
            validationLoss, valAccuracy = trainEpoch(model, validationLoader, scheduler, freezeModel=True)
    ...
    with torch.no_grad():
        testLoss, testAccuracy = trainEpoch(model, testLoader, scheduler, freezeModel=True)

    print(f'Accuracy of the network on the {int(RealVsFake140k.TEST_SIZE*SPLIT_FRACTION)} test images: {100 * testAccuracy}%, loss was {testLoss}')
